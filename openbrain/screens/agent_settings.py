...
class AgentSettingsScreen(ModalScreen[dict]):
    def compose(self) -> ComposeResult:
        ...
        yield Container(
            ...
            Horizontal(Label("Force tool use on first turn"), Input(id="force_tool_use")),
            Horizontal(Label("Retry limit"), Input(id="force_tool_retry_limit")),

            Label("Calculate Guard", classes="title"),
            Horizontal(Label("Nudge after N calculate-only calls"), Input(id="calc_guard_enabled")),
            ...
            Label("Difficulty Routing", classes="title"),
            Horizontal(Label("Enable routing"), Input(id="routing_enabled")),
            Horizontal(Label("Word threshold"), Input(id="routing_word_threshold")),
            Horizontal(Label("Keyword boost"), Input(id="routing_keyword_boost")),
    ...
    def on_mount(self) -> None:
        ...
        self.query_one("#force_tool_use", Input).value = str(cfg.force_tool_use).lower()
        self.query_one("#force_tool_retry_limit", Input).value = str(cfg.force_tool_retry_limit)
        self.query_one("#calc_guard_enabled", Input).value = str(cfg.calc_guard_enabled).lower()
        ...
        self.query_one("#routing_enabled", Input).value = str(cfg.routing_enabled).lower()
        self.query_one("#routing_word_threshold", Input).value = str(cfg.routing_word_threshold)
        self.query_one("#routing_keyword_boost", Input).value = str(cfg.routing_keyword_boost).lower()
        try:
    ...
    def key_ctrl_s(self) -> None:
        self.dismiss({
            ...
            "force_tool_use": self._str_to_bool(self.query_one("#force_tool_use", Input).value or "true"),
            "force_tool_retry_limit": int(self.query_one("#force_tool_retry_limit", Input).value or 1),
            "calc_guard_enabled": self._str_to_bool(self.query_one("#calc_guard_enabled", Input).value or "true"),
            ...
            "routing_enabled": self._str_to_bool(self.query_one("#routing_enabled", Input).value or "false"),
            "routing_word_threshold": int(self.query_one("#routing_word_threshold", Input).value or 200),
            "routing_keyword_boost": self._str_to_bool(self.query_one("#routing_keyword_boost", Input).value or "true"),
